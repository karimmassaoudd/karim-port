        ...
        <PageAnimator>
          ...
          <section ref={heroRef} id="hero" className="group relative flex flex-col justify-center h-lvh overflow-visible">
            ...
            <div className="text-center relative z-10">
              ...
              <h4 className="portfolio-subtitle hero-fade hero-fade-delay">{pageData.hero.subtitle}</h4>
              <div className="mt-6 sm:mt-8 hero-fade hero-fade-delay flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center px-4">
                <a href={pageData.hero.primaryButtonLink} className="btn btn-primary text-sm py-2 px-6">
                  {pageData.hero.primaryButtonText}
                </a>
                <a href={pageData.hero.secondaryButtonLink} className="btn text-sm py-2 px-6 backdrop-blur-md bg-white/10 border border-white/20 rounded-lg hover:bg-white/20 transition-all duration-300 text-[var(--text)] font-medium text-center">
          ...
          <section id="about" className="reveal-section relative py-12 sm:py-16 md:py-20 px-4 sm:px-6 lg:px-8 min-h-screen flex items-center overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-12 md:gap-16 items-center">
              <div>
                <div>
                  <h4 className="mb-2 text-right body-text-b reveal-el">{pageData.about.sectionLabel}</h4>
                  <div className="w-153 h-px bg-[var(--secondary-text)] opacity-50 mb-2" />
                  <h3 className="font-primary mb-6 pop-on-scroll">{pageData.about.heading}</h3>
                </div>
                <div className="min-h-[150px]">
                  <p 
                    className="text-base sm:text-lg text-[var(--hero-about-text)] mb-4 align-middle pop-on-scroll leading-relaxed"
                    dangerouslySetInnerHTML={{ __html: pageData.about.mainText }}
                  />
                </div>
                <div className="flex flex-col sm:flex-row items-start sm:items-center sm:space-x-6 space-y-4 sm:space-y-0 sm:justify-between">
                  <div className="flex gap-6">
                    <a href={`tel:${pageData.about.phoneNumber}`} className="text-[var(--accent)] hover:text-btn-primary icon-link" aria-label="Call me">
                      <MdLocalPhone size={24} />
                    </a>
                    <a href={`mailto:${pageData.about.email}`} className="text-[var(--accent)] hover:text-btn-primary icon-link" aria-label="Email me">
                      <IoMdMail size={24} />
                    </a>
                    <a href={pageData.about.linkedinUrl} target="_blank" rel="noopener noreferrer" className="text-[var(--accent)] hover:text-btn-primary icon-link" aria-label="My LinkedIn profile">
                      <FaLinkedin size={24} />
                    </a>
                  </div>
                  <div>
          ...
          <section id="user-experience" className="reveal-section bg-[var(--Secondary-Background)] text-[var(--text)] py-12 sm:py-16 md:py-20 px-4 sm:px-6 lg:px-8 relative">
            ...
            <div className="max-w-5xl mx-auto">
              <div className="text-right mb-6">
                <h4 className='body-text-b'>{pageData.userExperience.sectionLabel}</h4>
                <div className="h-px bg-[var(--secondary-text)] ml-auto max-w-[700px]" />
                <h3 className="text-2xl sm:text-3xl md:text-4xl text-left font-primary mb-8 sm:mb-12 max-w-[700px] w-full ml-auto reveal-el">{pageData.userExperience.heading}</h3>
              </div>
              <div>
