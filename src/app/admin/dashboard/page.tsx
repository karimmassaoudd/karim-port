...
export default function DashboardPage() {
  ...
  const fetchData = async () => {
    try {
          ...
          header: result.data.header || {
            ...
            projectDropdownLinks: [
              ...
              },
              {
                ...
                description: "Student guide platform",
                isVisible: true,
              },
              {
                id: 5,
                ...
                description: "Artist branding campaign",
                isVisible: true,
              },
            ],
          },
      ...
      // Error is already displayed via setMessage
      setMessage({ type: "error", text: "Failed to load data" });
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Silently handle save errors to prevent console flooding
      setMessage({ type: "error", text: "Failed to save changes" });
    } finally {
      setSaving(false);
    }
  ...
  const addDropdownLink = useCallback(() => {
    setFormData((prev) => {
      const newId =
        Math.max(...prev.header.projectDropdownLinks.map((l) => l.id), 0) + 1;
      return {
        ...prev,
        header: {
          ...prev.header,
  ...
  const removeDropdownLink = useCallback(
    (index: number) => {
      setFormData((prev) => {
        const newLinks = prev.header.projectDropdownLinks.filter(
          (_, i) => i !== index,
        );
        return {
          ...prev,
  ...
  return (
    <div className="flex h-[calc(100vh-theme('spacing.24'))] relative overflow-hidden bg-[var(--background)] max-w-7xl mx-auto rounded-xl border border-[var(--border)] shadow-2xl">
      {/* Sidebar Navigation */}
      <aside className="w-64 flex-shrink-0 bg-white/60 dark:bg-black/40 backdrop-blur-xl border-r border-[var(--border)] flex flex-col relative z-20">
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-1" ref={tabsRef}>
          {[
            ...
            >
              <item.icon
                className={`text-lg ${activeSection === item.id ? "text-white" : "text-gray-800 dark:text-gray-400"}`}
              />
              {item.label}
            </button>
          ))}
      ...
      {/* Main Content Area */}
      <div className="flex-1 flex flex-col overflow-hidden bg-white/50 dark:bg-black/40 backdrop-blur-xl relative z-20">
        <form onSubmit={handleSubmit} className="flex flex-col h-full relative">
          {/* Sticky Action Header */}
          <div className="sticky top-0 z-20 flex items-center justify-between px-8 py-4 bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-[var(--border)]">
            <div>
              <h2 className="text-xl font-bold text-[var(--headline)] capitalize flex items-center gap-2">
                {activeSection === "hero" && (
                  <MdHome className="text-[var(--accent)]" />
                )}
                {activeSection === "bio" && (
                  <MdPerson className="text-[var(--accent)]" />
                )}
                {activeSection === "about" && (
                  <MdInfo className="text-[var(--accent)]" />
                )}
                {activeSection === "experience" && (
                  <MdWork className="text-[var(--accent)]" />
                )}
                {activeSection === "projects" && (
                  <MdFolder className="text-[var(--accent)]" />
                )}
                {activeSection === "header" && (
                  <MdMenu className="text-[var(--accent)]" />
                )}
                {activeSection === "settings" && (
                  <MdSettings className="text-[var(--accent)]" />
                )}
            ...
            <div className="flex items-center gap-4">
              {message && (
                <span
                  className={`text-sm font-medium ${message.type === "error" ? "text-red-500" : "text-emerald-500"}`}
                >
                  {message.text}
                </span>
              )}
              <button
                type="submit"
                disabled={saving}
                className="flex items-center gap-2 px-6 py-2 bg-[var(--accent)] hover:bg-opacity-90 text-white rounded-lg font-bold text-sm shadow-md transition-all disabled:opacity-50"
              >
                {saving ? (
          ...
          <div ref={contentRef} className="flex-1 overflow-y-auto p-8 relative">
            <div className="max-w-4xl mx-auto">
              {/* Hero Section */}
              <div
                className={activeSection === "hero" ? "space-y-5" : "hidden"}
              >
              ...
              {/* About Section */}
              <div
                className={activeSection === "about" ? "space-y-5" : "hidden"}
              >
                <h3 className="text-sm font-bold text-[var(--headline)] mb-6 tracking-wide uppercase">
                  About Section
                </h3>
              ...
              <div
                className={
                  activeSection === "experience" ? "space-y-5" : "hidden"
                }
              >
                ...
                {formData.userExperience.items.length === 0 ? (
                  <div className="text-center py-12 bg-white/90 dark:bg-[var(--card)]/60 rounded-xl border border-white/30 dark:border-white/20 shadow-md">
                    <p className="text-xs font-secondary text-gray-600 dark:text-gray-300">
                      No experience items yet. Click "+ Add Experience" to
                      create one.
                    </p>
                  </div>
                ) : (
                      ...
                      <div className="space-y-2">
                        {formData.userExperience.items.map((item, index) => (
                          <button
                            ...
                            className={`w-full text-left px-3 py-3 rounded-lg font-secondary text-xs transition-all ${
              ...
              <div
                className={
                  activeSection === "projects" ? "space-y-5" : "hidden"
                }
              >
              ...
              {/* Header Section */}
              <div
                className={activeSection === "header" ? "space-y-5" : "hidden"}
              >
                <h3 className="text-sm font-bold text-[var(--headline)] mb-6 tracking-wide uppercase">
                  Header Navigation
                </h3>
                  ...
                  {formData.header.mainLinks.length > 0 ? (
                    <div className="space-y-3 mb-5">
                      {formData.header.mainLinks.map((link, index) => (
                        <div
                          key={`nav-${link.id}`}
                          className={`bg-white/80 dark:bg-gray-800/30 rounded-lg p-3 border transition-all cursor-pointer ${
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setFormData((prev) => ({
                                  ...
                                  header: {
                                    ...prev.header,
                                    mainLinks: prev.header.mainLinks.map(
                                      (l, i) =>
                ...
                <div className="bg-white/90 dark:bg-[var(--card)]/70 rounded-xl shadow-lg border border-white/40 dark:border-white/30 p-6">
                  ...
                  {formData.header.projectDropdownLinks.length > 0 ? (
                    <div className="space-y-3 mb-5">
                      {formData.header.projectDropdownLinks.map(
                        (link, index) => (
                          <div
                            key={`dropdown-${link.id}`}
                            className={`bg-white dark:bg-[#1a1a1a] rounded-lg p-4 border transition-all cursor-pointer shadow-sm ${
                    ...
                    <p className="text-xs font-secondary text-gray-500 dark:text-gray-400 mb-4">
                      No dropdown links yet. Click "Add Project Link" to create
                      one.
                    </p>
                  )}
                  ...
                  {selectedDropdownLinkIndex !== null &&
                    formData.header.projectDropdownLinks[
                      selectedDropdownLinkIndex
                    ] && (
                      <div className="backdrop-blur-sm bg-gradient-to-br from-[var(--accent)]/5 to-transparent rounded-lg p-4 border border-[var(--accent)]/20 space-y-4">
                        <h5 className="text-xs font-secondary font-bold text-headline">
                          Edit Project Link
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              ...
                              onChange={(e) =>
                                setFormData((prev) => ({
                                  ...
                                  header: {
                                    ...prev.header,
                                    projectDropdownLinks:
                                      prev.header.projectDropdownLinks.map(
                                        (l, i) =>
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className="w-full px-4 py-2 rounded-lg font-secondary backdrop-blur-sm bg-white/70 dark:bg-gray-800/50 text-gray-900 dark:text-white border border-white/30 dark:border-white/20 focus:border-[var(--accent)] focus:outline-none transition-all"
                              placeholder="e.g., /projects/travel-world"
                            />
                          </div>
                        </div>
                        <div>
              ...
              <div
                className={
                  activeSection === "settings" ? "space-y-5" : "hidden"
                }
              >
                <h3 className="text-sm font-bold text-[var(--headline)] mb-6 tracking-wide uppercase">
                  Settings
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <div className="flex gap-3 items-start">
                      ...
                      <label
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg font-secondary font-semibold bg-[var(--accent)]/20 text-[var(--accent)] border border-[var(--accent)]/30 hover:bg-[var(--accent)]/30 hover:shadow-lg transition-all cursor-pointer ${
                    ...
                    <p className="mt-2 text-xs text-[var(--secondary-text)]">
                      Upload an image from your computer (max 5MB). Supported
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-secondary font-semibold text-[var(--secondary-text)]">
                      OR
                    </span>
                    <div className="flex-1 h-px bg-white/20"></div>
                  </div>
