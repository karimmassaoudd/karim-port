...
import { MdLock, MdVisibility, MdVisibilityOff, MdSave, MdPerson, MdEmail } from 'react-icons/md';
import Toast from '@/components/Toast';

export default function SettingsPage() {
  const { data: session } = useSession();
  const [activeSection, setActiveSection] = useState<'account' | 'password' | 'email'>('account');
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.data.configured) {
          setEmailMessage({ type: 'success', text: '✓ Email service is configured and active' });
        }
  ...
  const handleEmailConfigSubmit = async (e: React.FormEvent) => {
    ...
    if (!gmailUser || !gmailAppPassword) {
      setEmailMessage({ type: 'error', text: 'Both Gmail user and app password are required' });
      return;
    }
  ...
  return (
    <>
      ...
      <div className="mb-6">
        <h3 className="text-base font-secondary font-bold text-headline mb-2">Settings</h3>
        <p className="text-xs font-secondary text-gray-600 dark:text-gray-300">Manage your account settings and preferences</p>
      </div>

      {/* Section Tabs */}
      <div ref={tabsRef} className="mb-6 backdrop-blur-xl bg-white/80 dark:bg-[var(--card)]/80 rounded-lg shadow-md border border-gray-300 dark:border-white/20 p-2">
      ...
      <div className="backdrop-blur-xl bg-white/80 dark:bg-[var(--card)]/80 rounded-lg shadow-sm border border-gray-300 dark:border-white/20 p-6">
        ...
        {activeSection === 'account' && (
          <div className="settings-card">
            <h3 className="text-xs font-secondary font-bold text-headline mb-4">Account Information</h3>
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-xs font-secondary font-bold text-headline">Change Password</h3>
            <p className="text-[10px] font-secondary text-gray-600 dark:text-gray-300">Update your password to keep your account secure</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 bg-gradient-to-br from-[var(--accent)] to-[var(--accent-hover)] rounded-lg flex items-center justify-center shadow-lg">
            <svg className="text-white text-xl" fill="none" stroke="currentColor" viewBox="0 0 24 24" width="24" height="24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
          </div>
          <div>
            <h3 className="text-xs font-secondary font-bold text-headline">Email Configuration</h3>
            <p className="text-[10px] font-secondary text-gray-600 dark:text-gray-300">Configure Gmail for password reset emails</p>
          </div>
        </div>
        ...
        <form onSubmit={handleEmailConfigSubmit} className="space-y-5">
          ...
          <div className="bg-[var(--accent)]/5 border border-[var(--accent)]/20 rounded-lg p-4">
            <h4 className="text-xs font-secondary font-semibold text-[var(--accent)] mb-2">How to get Gmail App Password:</h4>
            <ol className="text-xs font-secondary text-gray-900 dark:text-white space-y-1 list-decimal list-inside">
              <li>Go to your Google Account settings</li>
              <li>Navigate to Security → 2-Step Verification (enable if not enabled)</li>
          ...
          >
            <MdSave className="text-lg" />
            <span>{emailLoading ? 'Saving...' : 'Save Email Configuration'}</span>
          </button>
        </form>
          </div>
        )}
