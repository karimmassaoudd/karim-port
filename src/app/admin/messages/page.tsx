  ...
  return (
    <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h3 className="font-secondary text-base font-bold text-headline mb-2">
            Contact Messages
          </h3>
          <p className="text-xs font-secondary text-gray-600 dark:text-gray-400">
            Reading from src/data/contacts.json
          </p>
        </div>
        <a
      ...
      {messages.length === 0 ? (
        <p className="font-secondary text-gray-600 dark:text-gray-400">
          No messages yet.
        </p>
      ) : (
        <div className="space-y-4">
          {messages.map((m) => (
            <div
              key={m.id}
              className="backdrop-blur-xl bg-white/80 dark:bg-[var(--card)]/80 rounded-lg shadow-sm border border-white/20 dark:border-white/10 p-4"
            >
              ...
              <div className="mt-2 text-sm font-secondary text-gray-900 dark:text-white whitespace-pre-wrap">
                {m.message}
              </div>
            </div>
          ))}
